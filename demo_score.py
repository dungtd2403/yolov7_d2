...
def save_human(res, img, img_name: str):
    ins = res["instances"]
    ...
    if ins.has("pred_masks"):
        ...
        for x in range(len(human_instances)):
            ...
            # print(type(bit_mask))
            temp_mask = bit_mask.cpu().numpy()
            # background[bit_mask[0]]  = 255.0
            background[temp_mask[0]]  = 255.0
            # print(background)
            ...
            print(img_name)
            if not cv2.imwrite(f"./test_outside/img{img_name}_instance_{x}_score_{score}.png", background):
                raise Exception("Could not write image")
            #Save bounding box
            bbox = masks_to_boxes(bit_mask).cpu().numpy()
            # print(bbox.shape)
            ...
            try: 
                ...
                # cv2.imwrite(f"./2207_outdoor/{img_name}_instance_{x}_score_{score}.png", imgarr)
                
                # print(crop)
                
            except: 
                print(f"instance invalid")
            ground_truth = img_name[0:4]
            # name_file = img_name + ".txt"
            #add instance to txt file
            # f = open("./predict_2207_conf=0.5/" + name_file, 'a+')
            # # 
            # f.write("Person {} {} {} {} {} {} {}_ins{} \n".format(x_min, y_min, x_max, y_max, ground_truth,score,  image_name, x))
            # f.close()
            # cv2.imwrite(f"/home/dung/KITTI/ALP/yolov7_fake/yolov7/test_quyen/img{img_name}_instance_{x}_score_{score}.png", background)
            # cv2.imwrite(f"./2207_outdoor/semantic_with_boxes/img{img_name}_instance_{x}_score_{score}.png", background)
            # cv2.imwrite(f"./2207_outdoor/img{img_name}_instance_{x}_score_{score}.png", background)
            cv2.imwrite(f"./test_outside/img{img_name}_instance_{x}_score_{score}.png", background)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        #   index_image = '{:0>6}'.format(str(i))
          res = save_human(res, im, image_name)
